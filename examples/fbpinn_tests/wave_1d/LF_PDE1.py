...
class LF_PDE1(PhysLoss):
    ...
    @tf.function
    def phys_loss(self, model: tf.keras.Model, tape: tf.GradientTape, x_in, noise, active_models, **kwargs):
    ...
    @tf.function
    def boundary_loss_1(self, model: tf.keras.Model, tape: tf.GradientTape, x_in, noise, active_models, **kwargs):
        """u(0, x) = x^2"""
        x_in = x_in[:, 1]
    ...
    @tf.function
    def boundary_loss_2(self, model: tf.keras.Model, tape: tf.GradientTape, x_in, noise, active_models,  **kwargs):
        """u_t(0, x) = 0"""
        x_in = x_in[:, 1]
        t = tf.zeros_like(x_in)
        x = tf.stack([t, x_in], axis=1)
