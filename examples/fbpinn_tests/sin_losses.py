...
def fbpinn_orig_sin(
    ...
    **kwargs
    ...
    u_x = tape.gradient(u, x)
    u_model = u_x - omega * tf.cos(omega * x)
    u_true = tf.zeros_like(u_model)
    phys_loss = tf.reduce_mean(tf.square(u_true - u_model))
...
@tf.function
def fbpinn_orig_sin_full(
    model: TensorflowFBPINN, tape: tf.GradientTape, x, omega: float = 1, **kwargs
):
    """du/dx = omega * cos(omega*x), y(0) = 0"""
    tape.watch(x)
...
@tf.function
def boundary_loss_full(model: TensorflowFBPINN, tape: tf.GradientTape, x, **kwargs):
...
@tf.function
def boundary_loss_full2(model: TensorflowFBPINN, tape: tf.GradientTape, x, **kwargs):
    """solution function for y'' + 100y = 0, y(0) = 0, y'(0) = 10"""
    ic_x = tf.constant([[0.0]])
    tape.watch(ic_x)
    ...
    u_ic_true = tf.ones_like(ic_x)
    ic_loss = tf.reduce_mean(tf.square(u_ic_true - u_x))

    return ic_loss


def fbpinn_orig_sin_cos(
    model: TensorflowFBPINN,
    tape: tf.GradientTape,
    x,
    block: Block,
    ...
    **kwargs
    ...
    """du/dx = omega * cos(omega*x) - sin(x), y(0) = 1"""
    x_norm = block.normalization(x)
    predicted = model(x_norm)
    predicted_unnorm: tf.Tensor = block.unnormalization(predicted)
    ...
    u_x = tape.gradient(u, x)
    u_model = u_x - omega * tf.cos(omega * x) + tf.sin(x)
    u_true = tf.zeros_like(u_model)
    phys_loss = tf.reduce_mean(tf.square(u_true - u_model))
...
def boundary_loss_sin_cos(
    ...
    **kwargs
    ...
    """initial condition for du/dx = omega * cos(omega*x) - sin(x), y(0) = 1"""
    ic_x = tf.constant([[0.0]])
    tape.watch(ic_x)
