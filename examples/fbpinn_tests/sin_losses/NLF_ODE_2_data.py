...
class NLF_ODE_2(PhysLoss):
    ...
    @tf.function
    def phys_loss(self, model: tf.keras.Model, tape: tf.GradientTape, x, noise, active_models, **kwargs):
        """du/dx = omega * cos(omega*x)"""
        tape.watch(x)

        u = model(x, active_models)
        u_x = tape.gradient(u, x)
        u_model = u_x - self.omega1 * tf.cos(self.omega1 * x) + self.omega2 * tf.sin(self.omega2 * x) + noise
    ...
    @tf.function
    def boundary_loss_1(self, model: tf.keras.Model, tape: tf.GradientTape, x, active_models, **kwargs):
        """y(0) = 1"""
        x = tf.constant([[0.0]])
        tape.watch(x)
