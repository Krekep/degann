...
class NLF_ODE_2(PhysLoss):
    def __init__(
        self, description: str = "", omega1: float = 10, omega2: float = 1, **kwargs
    ):
        description = (
            "du/dx = omega1 * cos(omega1*x) - omega2 * sin(omega2 x), y(0) = 1"
        )
        self.omega1 = omega1
        self.omega2 = omega2
    ...
    def phys_loss(
        ...
        **kwargs
        ...
        u_model = (
            u_x
            - self.omega1 * tf.cos(self.omega1 * x)
            + self.omega2 * tf.sin(self.omega2 * x)
            + noise
        )
    ...
    def first_der(self, x):
        return self.omega1 * tf.cos(self.omega1 * x) - self.omega2 * tf.sin(
            self.omega2 * x
        )
