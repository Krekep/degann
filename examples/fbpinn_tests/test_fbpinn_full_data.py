...
data_for_noise = tf.sort(nn.data, axis=0)
indices = [
    200,
    len(nn.blocks) * model_config["points_per_block"] // 2,
    len(nn.blocks) * model_config["points_per_block"]
    - model_config["points_per_block"] // 2,
]
for idx in indices:
    noise_py[idx] = ode.solution(data_for_noise[idx]).cpu().numpy().item() * 0.2
with tf.device("/GPU:0"):
    noise = tf.constant(noise_py, shape=data_for_noise.shape, dtype=tf.float32)
nn.train(
    **train_config,
    callbacks=None,
    verbose=0,
...
y_model_true = ode.solution(data_for_noise)
y_noise = y_model_true + noise
axes[1].plot(data_for_noise, y_noise, label="Real data + noise", color="red")
y_pred = nn(data_for_noise)
axes[1].plot(data_for_noise, y_pred, label="Predicted", color="green")
axes[1].scatter(
    tf.gather(data_for_noise, indices=indices),
    tf.gather(y_noise, indices=indices),
    label="Noise offset",
    color="blue",
)
axes[1].set_xlabel("x")
...
axes[1].legend()
axes[1].grid()
plt.savefig(f"FBPINN_{str(run_id)}.png", dpi=300, bbox_inches="tight")
plt.show()
