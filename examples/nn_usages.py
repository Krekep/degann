...
from networks import trainer, losses, callbacks
...
#
weight_initializer1, bias_initializer = init_params(weight_name='ones', bias_name='zeros')

#
# Create simple neural network with 1 input neuron, 1 output neuron and one hidden layer with 1 neuron
#
nn = IModel.create_neuron(1, 1, [1], weight=weight_initializer1, biases=bias_initializer)
...
print(f"Last layer have weights {nn.network.out_layer.w.numpy()} and biases {nn.network.out_layer.b.numpy()}")
...
nn_loaded.from_file("./test_export")  # restore neural network from file (now weights must be equal to nn weights)
...
x_data = np.array([[i / 10] for i in range(0, 101)])  # X data, 100 points on [0, 10] with unifrom distribution
...
input_size = 1
shapes = [[10, 10, 10, 10, 10, 10], [100, 100, 100], [300, 300, 300]]  # sizes of hidden layers
output_size = 1
...
for i, shape in enumerate(shapes):
    ...
    for size in single_x_data_call:
        ...
        print(f"Neural network with {shape} have predict row by row time {call_time} on {len(size)} size")
