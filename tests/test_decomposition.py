...
def test_decomposition(left_corner, right_corner, block_size, overlap, expected_blocks_count):
    domain = RectangleDomain(left_corner, right_corner)
    actual_decomposition = Decomposition(domain, overlap, block_size)

    assert reduce(lambda x, y: x * y, actual_decomposition.blocks_per_axis) == len(actual_decomposition.blocks)
